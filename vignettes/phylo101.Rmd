...
First, we need to add the path to the state flag icons to the metadata using **to_image()**

```{r}
d <- to_image(d, "state", "states")
d
```

```{r}
us_state_flag_tree(t1, d, image_column = "ImagePath", label_column = "accession")
```
